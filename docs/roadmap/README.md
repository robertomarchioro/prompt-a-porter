...
| [`ordito-sync-log.md`](./ordito-sync-log.md) | 🧭 *Esplorazione post-1.0 (Deluxe→Enterprise)* — blueprint "Ordito": oplog replicato firmato (HLC, LWW per campo, conflitto→variante), sync multi-device senza server (cartella/LAN/relay muto), DB come proiezione → strato database-agnostic per v2.x |
