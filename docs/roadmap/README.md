...
| [`ordito/`](./ordito/blueprint-F1.md) | ✏️ *Blueprint operativi Ordito (design, no codice)* — tagli esecutivi delle 4 fasi: **F1** Fondamenta (DDL V016, HLC, `apply_change`, key-storage, GC locale, push riparato, 8 PR), **F2** Trasporto cartella (VSK/recovery phrase, segmenti cifrati, viste firmate, snapshot a quorum, stale-out), **F3** LAN P2P (Noise XX, negentropy, variante-da-conflitto, Vista conflitti, UseCount G-counter, relay T3 opz.), **F4** Enterprise (staging+catena workspace, certificati device→utente, proiezione Go SQLite/Postgres, conformance suite — gated dal cliente) |
