...
| **Collezioni — importer CSV generico** | 📋 → v1.x | Decisione D4 di [`collezioni-curate.md`](./collezioni-curate.md): un `vault_import_csv` (`titolo,corpo`) coprirebbe le fonti aperte tipo awesome-chatgpt-prompts senza connettori per sito. Si fa solo se qualcuno lo chiede. |
| **Embedding stale dopo `aggiorna`/`overwrite` in import** | 🎨 → v1.x | `import_pure` non ha accesso a `EmbeddingsState`: un corpo sovrascritto (collezioni «Aggiorna», import JSON `overwrite`) resta con l'embedding vecchio finché non viene salvato dall'editor; il backfill copre solo i prompt senza righe. Stesso ceppo del bullet sopra (#303). Colto in review del passo 2 collezioni. |
| **Import — tag omonimo soft-deleted** | 📋 → con `tags.rs` | `UNIQUE (WorkspaceId, Name)` su `Tags` non è parziale: vale anche per le righe con `DeletedAt`. `tag_id_per_nome` in `import_pure` cerca solo i tag attivi, quindi un tag omonimo *cestinato* farebbe fallire l'INSERT e perdere l'associazione. Oggi irraggiungibile (nessun comando utente cancella tag; solo `sync.rs` Ordito). Da chiudere insieme al backend `tags.rs`: indice unique parziale `WHERE DeletedAt IS NULL` oppure ripristino del tag cestinato in import. Colto in review della PR collezioni. |
| **Collezioni — scelta nell'onboarding** | 📋 → v1.x | Oggi le collezioni si importano da Impostazioni → Dati. Offrirle nel wizard accanto al demo vault (#284) tocca un flusso collaudato dal vivo: PR separata, dopo la prova dal vivo della card. |
