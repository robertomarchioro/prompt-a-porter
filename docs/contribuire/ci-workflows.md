...
| `client-build.yml` | `push:main` + `pull_request` su `apps/client/**` ∪ `packages/**` ∪ `pnpm-workspace.yaml` ∪ `pnpm-lock.yaml` ∪ `.github/workflows/client-build.yml` ∪ `docs/collezioni/**` ∪ `scripts/collezioni-indice.mjs` (no md) + `workflow_dispatch` | `lint-and-test` (TypeScript, **coverage gate 70%**), `rust-test` (`cargo check --all-targets` + cargo-llvm-cov **gate 80% line**, `--locked`) |
...
| `docs/collezioni/**` o `scripts/collezioni-indice.mjs` | **client-build** (eccezione alla regola `docs/**`: i test `collezioni::*` includono i JSON e rifiutano un `indice.json` stale — senza questo trigger una collezione modificata senza rigenerare l'indice mergerebbe verde e romperebbe l'import per tutti) |
