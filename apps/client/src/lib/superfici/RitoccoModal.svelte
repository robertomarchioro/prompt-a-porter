...
>
  ...
  {:else if fase === "esecuzione"}
    <div class="ritocco-loading">
      <span class="spinner" aria-hidden="true"></span>
      <p>Il sarto è al lavoro…</p>
    </div>
  ...
  {#snippet footer()}
    {#if !gating}
      <button type="button" class="secondaria" onclick={onChiudi}>Chiudi</button>
    {:else if fase === "scelta"}
      <button type="button" class="secondaria" onclick={onChiudi}>Annulla</button>
      <button type="button" class="primaria" onclick={() => void avvia()}>
        Avvia Ritocco
      </button>
    {:else if fase === "esecuzione"}
      <button type="button" class="primaria" disabled>In corso…</button>
    {:else}
      <button type="button" class="secondaria" onclick={onChiudi}>Annulla</button>
      <button
        type="button"
...
<style>
  ...
  .hint {
    color: var(--testo-attenuato, #888);
    font-size: 0.9rem;
    margin: 0;
  }
  .err {
    color: var(--errore, #d93f0b);
    font-size: 0.9rem;
    margin: 0;
  }
  .warn {
    color: var(--attenzione, #b8860b);
    font-size: 0.9rem;
    margin: 0;
  }
  .campo {
  ...
  .spinner {
    width: 28px;
    height: 28px;
    border: 3px solid var(--bordo, #ccc);
    border-top-color: var(--accento, #646cff);
    border-radius: 50%;
